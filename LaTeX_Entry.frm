...
'   Dim WebAdd
    ...
    ' Base_Web_Address = "http://engineerolabs.github.io/Process_LaTeX/"
    ...
    '         String_Stop_Loc - String_Start_Loc - 16)
        ...
        File_Name = ActiveDocument.Path & "\tmp.png"

        src = ActiveDocument.Path & "\tmp.tex"
        Open src For Output As #1
        Print #1, Latex_Str
        Close #1

        opt = "" ' --force-node-api-uncaught-exceptions-policy=true"
        cmd = "%ComSpec% /c " & ThisDocument.TEX2IMG & opt & " " & src & " " & File_Name
    ...
    Else
        ...
        Do Until objExec.StdOut.AtEndOfStream
            err_msg = err_msg & objExec.StdOut.ReadLine & vbCrLf
        Loop

        Report_Error.Error_Text.Text = err_msg
        Load Report_Error
        Report_Error.Show
